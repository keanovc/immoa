...
const PublicHeader = () => {
    ...
    return (
        auth ? (isAdmin(auth.user)) ?        
            <>
            <Navbar navItems={NavItems} dashboard={HomeRoutes.Index} handleToggle={handleToggle} toggle={toggle}>
            {
                <>
                    <button onClick={handleToggleProfile} id="dropdownDefault" data-dropdown-toggle="dropdown" className="flex justify-center items-center outline-none space-x-3 cursor-pointer bg-transparent px-5 py-2 rounded-md" type="button">
                        <div className="w-12 h-12 flex items-center justify-center bg-slate-700 rounded-full overflow-hidden border-2">
                            <FaUserCog className="text-2xl" />
                        </div>    
                    </button>
                    <div id="dropdown" className={`absolute w-44 px-5 py-3 backdrop-blur-md bg-white/30 bg-white rounded-lg shadow border dark:border-transparent top-20 right-5 md:right-auto z-10 ${toggleProfile ? '' : 'hidden'}`}>
        ...
        : isRealtor(auth.user) ?
            ...
            <Navbar navItems={NavItems} dashboard={HomeRoutes.Index} handleToggle={handleToggle} toggle={toggle}>
            {
                <>
                    ...
                    <Link to={AuthRoutes.Signin} className="md:hidden bg-gray-700 flex items-center justify-center w-12 h-12 rounded-full overflow-hidden border-2 dark:border-white border-gray-900 mr-5">
